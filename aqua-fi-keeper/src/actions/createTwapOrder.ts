...
import { COIN_TYPE_SUI, COIN_TYPE_USDC } from '../utils/constants';
...
export interface TwapOrderConfig {
    totalAmount: number;    // Montant total en MIST
    totalOrders: number;    // Nombre total de swaps
    intervalSeconds: number; // Intervalle en secondes
}

async function createTwapOrderImpl(config: TwapOrderConfig) {
    ...
    try {
        ...
        if (coinBalance > config.totalAmount) {
            const [splitCoin] = txb.splitCoins(
                txb.object(selectedSuiCoin.objectId),
                [txb.pure.u64(config.totalAmount)]
            );
        ...
        if (result.effects?.status.status === 'success') {
            ...
            if (createdObjects && createdObjects.length > 0) {
                const orderId = (createdObjects[0] as any).objectId;
...
export async function cancelTwapOrder(orderId?: string) {
...
async function getCoinBalance(coinId: string): Promise<number> {
    try {
...
export async function createTwapOrder(config: TwapOrderConfig) {
